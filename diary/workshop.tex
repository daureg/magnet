...
\item The \alert{trollness} of $i$ $tr(i)$ is its fraction of negative outgoing links, its
\alert{untrustworthiness} \(un(i)\) is its fraction of negative incoming links.
\end{itemize}
  ...
  \begin{center}
    \begin{tikzpicture}[auto,scale=1.2, every node/.style={transform shape}]
      ...
      \node[peers] (i) at (0,0) {\LARGE $i$};
      \node[peers] (j) at (3.5,-0.66) {\Large $j$};

      \draw[edge] (i) -- node [posc,xshift=-2mm] {\Large $+$}                      (3, -2);
      \draw[edge] (i) edge  node [posc,xshift=-2mm] {\Large $\textcolor{Black}{y_{ij}}=\textcolor{Green}{+}$} (j);
      \draw[edge] (i) edge  node [posc,xshift=2mm] {\Large $+$}                    (3, 0.66);
      \draw[edge, densely dashed] (i)   edge   node [negc,xshift=2mm] {\Large $-$} (3, 2);

      \draw[edge, densely dashed] (-3, -2) --   node [negc,yshift=-3mm] {\Large $-$} (i);
      \draw[edge, densely dashed] (-3, -1) --   node [negc,yshift=-1mm] {\Large $-$} (i);
      \draw[edge, densely dashed] (-3, 0)  --  node [negc] {\Large $-$} (i);
\draw[edge,]                (-3, 1) --   node [posc,xshift=-4mm] {\Large $+$} (i);
\draw[edge,]                (-3, 2) --   node [posc,below,pos=0.2] {\Large $+$} (i);
\node[fill=white!90!DarkOrange,fill opacity=.9,text opacity=1,rounded corners,inner sep=1pt] at (1,-1.5) {$tr(i)=\frac{1}{4}$};
\node[fill=white!90!Blue,fill opacity=.5,text opacity=1,rounded corners,inner sep=1pt] at (-1.0,1.7) {$un(i)=\frac{3}{5}$};
\end{tikzpicture}
  \end{center}
...
\begin{frame}{Generative model}
  \begin{center}
    ...
    \node[] (iparams) at (1.2,1.1) {$\outqt{p_i}, q_i \begin{cases} \sim \mu(p, q) &\\ \in [0, 1]&\end{cases}$};

      \node[peers,] (j) at (7,0) {$j$};
    \node[] (jparams) at (8.2,1.1) {$p_j, \inqt{q_j} \begin{cases} \sim \mu(p, q) &\\ \in [0, 1]&\end{cases}$};
  ...
  $$y^*(i,j) = \sgn\big(\Pr\big( y_{i,j} = +1 \big) - \frac{1}{2}\big)$$}
\end{frame}
