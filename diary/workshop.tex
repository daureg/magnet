...
\begin{frame}{Generative model}
  \begin{center}
  \onslide<1->{
    \includegraphics<1>[height=4cm]{gen_model_noedge.pdf}
    \includegraphics<2->[height=4cm]{gen_model_edge.pdf}
  }
\end{center}
...
\outqt{\(1-\htr(i)\)} is an indirect observation of \outqt{$p_i$}. Letting $\biasqt{\qbar_i}$ be the
...
\tightlist
\item
  If possible, sample\footnote{uniformly at random without replacement.} \outqt{$Q$ edges outgoing from each
  node $i$} and let \(\htr(i) = \hdout^-(i)/Q\);
\item
  If possible, sample \inqt{$Q$ edges ingoing to each node $i$} and let \(\hun(i) = \hdin^-(i)/Q\);
\item
  Sample any edge in \(E_L\) not yet sampled, and let \(\tauhat\) be \biasqt{the fraction of positive
  edges in \(E_L\)};
\item
...
$\Psiout(i,Y) + O\left(\sqrt{\Psiout(i,Y)}+ 1\right)$ mistakes on average~\autocite{acg02}.
    ...
    \item 
The meta expert \outqt{\(RWM_{out}\)} predicts \(y_{i,j}\) as \(RWM_{out}(i)\) and makes on average
$\Psiout(Y) + O\left(\sqrt{|V|\Psiout(Y)} + |V|\right)$ mistakes.

    \item 
    Combining \outqt{\(RWM_{out}\)} and a similarly constructed \inqt{\(RWM_{in}\)} gives our final
